function [outfilefullname, num_ch_entrained] = Analysis32ch072220(cleaned_file,analyzed_dir)
...
disp(['Analyzing: ', cleaned_file]);
% input_dir = 'C:\Users\matto\Box\Project_FlickerBloodFlow\EEG\CABI Recordings';
% clean_folder = '\CleanedData';

%[filepath,name,ext] = fileparts(filename) returns the path name, file
%name, and extension for the specified file. fileparts only parses the
%specified filename. It does not verify that the file exists.
[filepath,input_filename,fileext] = fileparts(cleaned_file);



% input_file = 'S002_01_09_f_1_200_butterord3_32ch'; %without .mat extension
out_filename = '_RESULTS';
montage = ['bipolar'];  % 'commonCz', 'average', bipolar'
...
if strcmp(montage,'average')
    channels = {'Fp1','AF3','F7','F3','FC1','FC5','T7','C3','CP1','CP5','P7','P3','Pz','PO3','O1','Oz','O2','PO4','P4','P8','CP6','CP2','C4','T8','FC6','FC2','F4','F8','AF4','Fp2','Fz','Cz'};
    % chanlocs = EEG.chanlocs(1:40);  % chanlocs = EEG.chanlocs(1:64);
    %     Fs = EEG.srate;
    % channels = extractfield(chanlocs, 'labels'); 
%     common_montage_ch = 32; %32 is Cz for 32ch
    results_folder = 'AverageMontage';  % place to save output
end

if strcmp(montage,'commonCz')
    channels = {'Fp1','AF3','F7','F3','FC1','FC5','T7','C3','CP1','CP5','P7','P3','Pz','PO3','O1','Oz','O2','PO4','P4','P8','CP6','CP2','C4','T8','FC6','FC2','F4','F8','AF4','Fp2','Fz','Cz'};
    % chanlocs = EEG.chanlocs(1:40);  % chanlocs = EEG.chanlocs(1:64);
    %     Fs = EEG.srate;
    % channels = extractfield(chanlocs, 'labels'); 
    common_montage_ch = 32; %32 is Cz for 32ch
    results_folder = 'CommonMontageCz';  % place to save output
end

if strcmp(montage,'bipolar')  % 32-channel Anterior-Posterior Bipolar Montage
    channels = {'Fz_Cz' 'Cz_Pz' 'Pz_Oz' 'Fz_FC1' 'FC1_CP1' 'CP1_Oz' 'Fz_FC2'...
        'FC2_CP2' 'CP2_Oz'... %center
        'Fp1_F7' 'F7_T7' 'T7_P7' 'P7_O1' 'Fp1_FC5' 'FC5_CP5' 'CP5_O1'...
        'Fp1_AF3' 'AF3_F3' 'F3_C3' 'C3_P3' 'P3_PO3' 'PO3_O1' ... %left
        'Fp2_AF4' 'AF4_F4' 'F4_C4' 'C4_P4' 'P4_PO4' 'PO4_O2'...
        'Fp2_FC6' 'FC6_CP6' 'CP6_O2' 'Fp2_F8' 'F8_T8' 'T8_P8' 'P8_O2'... %right
        };  
    results_folder = 'BipolarMontage\AutoClean';  % place to save output
end
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% load data file
% load([input_dir clean_folder '\' input_file '.mat']);
load(cleaned_file);
...
%     end
    ...
    % too stringent, requires peak to be above both
...
%     end
    ...
    if std_above_mean(i) > 3
        entrained_ch = [entrained_ch i];
    end
        
end

num_ch_entrained = length(entrained_ch);
fprintf('Number of channels entrained at %d Hz is %d (out of %d total)\n', peak_discrete_freq, num_ch_entrained, length(channels));
...
% % Create figures subfolder  (makes folder in wrong location)
% if exist( [filepath results_folder],'dir') == 0
%     mkdir([filepath results_folder])
% end

outfilefullname = [analyzed_dir results_folder '\' input_filename out_filename '.mat'];    
save(outfilefullname, ...
'eeg', 'aux_data', 'samp_freq', 'unused_eeg_channels', 'eeg_channel_labels', ...
'aux_labels', 'rej_time', 'event_stop_times_pts', 'event_stop_times','std_above_mean','num_ch_entrained');

disp(['Completed analyzing: ', cleaned_file, ' .  Results file saved as: ' outfilefullname]);
end
