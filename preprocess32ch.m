function [out_file] = preprocess32ch(input_file,output_dir)
...
% editted by MKA 1-24-2020, adapted from SM
%user defined variables
% input_dir = 'C:\Users\matto\Box\Project_FlickerBloodFlow\EEG\CABI Recordings\rawdata\Experiment01\S002';
% input_file = 'S002_01_09';  %without .bdf extension
% output_dir = 'C:\Users\matto\Box\Project_FlickerBloodFlow\EEG\CABI Recordings\CleanedData';

% for config file 64+8, 32+8
eeg_channels = 1:32; % eeg_channels = [1:64];  
aux_channels = 33:40; %Change to 33:40 for EEG, old auxiliary channels [73 74]  
aux_labels = {'aux1', 'aux2','aux3','aux4','aux5','aux6','aux7','aux8',};  %{'photodiode', 'microphone'}
...
disp_channel_labels = 0; % 1 to display channel labels in eegplot; 0 to display channel number, DONT CHANGE
art_remove = 0; % 1 to do manual artifact removal w/ GUI, 0 to skip artifact removal
...
disp(['Processing: ', input_file]);
%% read in EEG file
time_range = [1 120]; % only take the first 2 mins
down_fs = 512; %downsample frequency to 
EEG = pop_biosig(input_file,'blockrange',time_range);
% EEG = pop_resample( EEG, down_fs);  %do not need to resample for cabi
% measurements 1-30
...
if art_remove
    if (disp_channel_labels == 1) %display EEG channel names
        eegplot(eeg_data_avg, 'srate', samp_freq, 'eloc_file', 'C:\Users\matto\Box\Project_FlickerBloodFlow\EEG\EEG Cap\Cap32.locs', 'title', 'Processed EEG data for artifact removal - average montage') 
        % *** standard cap 32
...
if art_remove
    ...
    error('SELECT ARTIFACT SEGMENTS MANUALLY, THEN GO TO FIGURE > ACCEPT&CLOSE, THEN EXECUTE REMAINDER OF THE SCRIPT FROM "Execute to save data" (LINE ~250)')
...
out_file = [output_dir '\' input_filename '_f_' num2str(eeg_locutoff) '_' num2str(eeg_hicutoff), '_' eeg_filter_type 'ord' num2str(eeg_filter_order) '_' num2str(length(eeg_channels)) 'ch' add_label '.mat'];
out_file2 = [output_dir '\' '_f_' num2str(eeg_locutoff) '_' num2str(eeg_hicutoff), '_' eeg_filter_type 'ord' num2str(eeg_filter_order) '_' num2str(length(eeg_channels)) 'ch' add_label '.mat'];
%save processed file
...
disp(['Done preprocessing: ', input_file, ' . Saved cleaneddata as: ', out_file])
